...
<table class="table">
  ...
  <tbody>
    <tr ng-repeat="x in names | filter:test">
        ...
        <td>{{ x.contact }}</td>
        <td><a href="Modification.jsp?iddoc={{ x.id  }}&&nom={{ x.nom }}&&prenom={{ x.prenom }}&&contact={{ x.contact }}">Modifier</a></td>
        
        <td><a href="Supprimer.jsp?iddoc={{ x.id  }}">Supprimer</a></td>
...
app.controller('customersCtrl', function($scope, $http) {
    $http.get("http://localhost:8084/Hopital/selectD.jsp").then(function (response) {$scope.names = response.data;
    });
});
</script>
